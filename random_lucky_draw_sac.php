...
<div class="container mx-auto px-4 flex-grow">
    <div class="card mb-8">
        <div class="text-center">
            ...
            <div id="prize-names" class="text-blue-500 font-bold text-xl md:text-3xl mb-4"></div>
...
<script>
    ...
    async function fetchRandomID() {
        const response = await fetch('fetch_random_name_sac.php');
        const data = await response.json();
        return data.id;
    }

    async function fetchFinalName() {
        const response = await fetch('fetch_final_name_sac.php');
        const data = await response.json();
        return data.winner_lucky_draw;
    }

    async function fetchWinnerInfo(name) {
        const response = await fetch('fetch_winner_info_sac.php', {
            method: 'POST',
            headers: {
    ...
    async function displayRandomNames() {
        ...
        for (let i = 0; i < 1; i++) {
            lastFinalName = await fetchFinalName();
            await new Promise(resolve => setTimeout(resolve, 100));
        }
    ...
    async function markWinner(id) {
        await fetch('mark_winner_sac.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
<!--script>
    async function fetchPrizeNames() {
        const response = await fetch('fetch_award_sac.php');
        const prizes = await response.json();

        // แสดงรายชื่อรางวัลใน element ที่มี id="prize-names"
        const prizeNamesDiv = document.getElementById('prize-names');

        prizeNamesDiv.innerHTML = prizes.map(prize => prize.prize_name).join('<br>');

    }

    // เรียกใช้ฟังก์ชันเมื่อโหลดหน้าเสร็จ
    document.addEventListener('DOMContentLoaded', fetchPrizeNames);
</script-->
